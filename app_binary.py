...
# --- 1. 初始化設定 ---
st.set_page_config(page_title="AI 肺炎影像篩檢系統 (二元精準版)", layout="wide")
...
st.markdown("本系統採用 Vision Transformer 模型，專注於提供**高精準度**的肺炎風險二元篩檢。")
...
uploaded_file = st.file_uploader("📂 請上傳胸部 X 光片 (支援 JPG, PNG)", type=["jpg", "jpeg", "png"])
    ...
    with col2:
        ...
        if confidence >= confidence_threshold:
            if pred_class == "NORMAL":
                ...
                st.write("影像清澈，未發現明顯肺部浸潤或發炎徵兆。")
            else:
                st.error("🔴 **檢測結果：發現肺炎徵兆 (Pneumonia Detected)**")
                st.warning("請盡速尋求專業胸腔科醫師進行進一步診斷。")
                
            st.metric("AI 判斷把握度", f"{confidence:.2%}")
        else:
            st.warning("⚠️ 影像特徵模糊")
            st.write("AI 把握度不足，建議重新拍攝或由人工判讀。")
            
    with col3:
        st.subheader("AI 病灶關注區域 (Grad-CAM)")
        st.image(visualization, use_container_width=True) 
        st.caption("紅色區域為 AI 判斷時高度關注的特徵位置。")